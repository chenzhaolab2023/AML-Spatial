...
def Generate_v2(Original_loc, Mask_loc, patient, fov, max_dim): # This function crops single cells segmented in mask image, putting them on a black background (each as own max_dim x max_dim x 3 numpy array)
    ...
    # NOTE: since we're only interested in the CD34 channel from the 3 channel normalized image, we only have the red channel of the cropped cell and triplicate it to create a psuedo-RGB
