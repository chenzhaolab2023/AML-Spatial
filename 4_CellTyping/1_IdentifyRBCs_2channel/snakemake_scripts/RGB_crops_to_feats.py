...
def Generate_v2(Original_loc, Mask_loc, patient, fov, max_dim): # This function crops single cells segmented in mask image, putting them on a black background (each as own max_dim x max_dim x 3 numpy array)
    
    ## inputs:
    # Original_loc = path to PNG image to be cropped
    ...
    for n in cell:
        if len(n) != 0:
            ...
            if cell_image[y0:y1, x0:x1, :].shape[0] > new_image[yo:yi, xo:xi, :].shape[0] or cell_image[y0:y1, x0:x1, :].shape[1] > new_image[yo:yi, xo:xi, :].shape[1]:
                ...
                # stack back to rgb
                img_bgr = np.stack([resized_img_ch0, resized_img_ch1, resized_img_ch2], axis=2)
...
cell_names, cell_stack = Generate_v2(snakemake.input[0], snakemake.input[1], snakemake.params[0], snakemake.params[1], 200)

# run feature extraction, save as DF
